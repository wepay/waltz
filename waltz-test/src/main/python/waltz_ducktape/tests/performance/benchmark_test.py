...
class BenchmarkTest(ProduceConsumeValidateTest):
    ...
    @parametrize(txn_size=512, txn_per_thread=1000, num_thread=10, interval=20, lock_pool_size=0, num_active_partitions=1, timeout=360)
    ...
    @parametrize(txn_size=1024, txn_per_thread=1000, num_thread=10, interval=10, lock_pool_size=0, num_active_partitions=1, timeout=360)
    @parametrize(txn_size=512, txn_per_thread=100, num_thread=10, interval=10, lock_pool_size=64, num_active_partitions=1, timeout=360)
    @parametrize(txn_size=512, txn_per_thread=100, num_thread=10, interval=10, lock_pool_size=128, num_active_partitions=1, timeout=360)
    @parametrize(txn_size=512, txn_per_thread=100, num_thread=10, interval=10, lock_pool_size=128, num_active_partitions=2, timeout=360)
    def test_producer_performance(self, txn_size, txn_per_thread, num_thread, interval, lock_pool_size, num_active_partitions, timeout):
        test_cmd = self.performance_cli.producer_test_cmd(self.verifiable_client.config_file_dir, txn_size, txn_per_thread, num_thread, interval, lock_pool_size, num_active_partitions)
    ...
    def test_consumer_performance(self, txn_size, num_txn, num_active_partitions, timeout):
        test_cmd = self.performance_cli.consumer_test_cmd(self.verifiable_client.config_file_dir, txn_size, num_txn, num_active_partitions)
        test_output = self.run_produce_consume_validate(lambda: self.simple_validation_func(test_cmd, timeout))
