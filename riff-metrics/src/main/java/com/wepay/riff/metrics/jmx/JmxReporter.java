...
import com.wepay.riff.metrics.core.MetricRegistryListener;
import com.wepay.riff.metrics.core.Reporter;
 ...
 */
public final class JmxReporter implements Reporter, Closeable {
    /**
    ...
    public static final class Builder {
        ...
        private Map<MetricId, TimeUnit> specificDurationUnits;
        private Map<MetricId, TimeUnit> specificRateUnits;
        ...
        /**
         * Use specific {@link TimeUnit}s for the duration of the metrics with these ids.
         *
         * @param specificDurationUnits a map of metric ids and specific {@link TimeUnit}s
         * @return {@code this}
         */
        public Builder specificDurationUnits(Map<MetricId, TimeUnit> specificDurationUnits) {
        ...
        /**
         * Use specific {@link TimeUnit}s for the rate of the metrics with these ids.
         *
         * @param specificRateUnits a map of metric ids and specific {@link TimeUnit}s
         * @return {@code this}
         */
        public Builder specificRateUnits(Map<MetricId, TimeUnit> specificRateUnits) {
    ...
    private static final class JmxGauge extends AbstractBean implements JmxGaugeMBean {
    ...
    private static final class JmxCounter extends AbstractBean implements JmxCounterMBean {
    ...
    private static final class JmxHistogram implements JmxHistogramMBean {
        private final ObjectName objectName;
    ...
    // though with private constructor, JmxMeter should not be declare
    // as final because JmxTimer class extends JmxMeter
    ...
    static final class JmxTimer extends JmxMeter implements JmxTimerMBean {
    ...
    private static final class JmxListener implements MetricRegistryListener {
        ...
        public void onGaugeAdded(MetricId id, Gauge<?> gauge) {
            try {
                if (filter.matches(id, gauge)) {
                    final ObjectName objectName = createName("gauges", id);
        ...
        public void onCounterAdded(MetricId id, Counter counter) {
            try {
                if (filter.matches(id, counter)) {
                    final ObjectName objectName = createName("counters", id);
        ...
        public void onHistogramAdded(MetricId id, Histogram histogram) {
            try {
                if (filter.matches(id, histogram)) {
                    final ObjectName objectName = createName("histograms", id);
        ...
        public void onMeterAdded(MetricId id, Meter meter) {
            try {
                if (filter.matches(id, meter)) {
                    final ObjectName objectName = createName("meters", id);
                    registerMBean(new JmxMeter(meter, objectName, timeUnits.rateFor(id)), objectName);
                }
            } catch (InstanceAlreadyExistsException e) {
        ...
        public void onTimerAdded(MetricId id, Timer timer) {
            try {
                if (filter.matches(id, timer)) {
                    final ObjectName objectName = createName("timers", id);
                    registerMBean(new JmxTimer(timer, objectName, timeUnits.rateFor(id), timeUnits.durationFor(id)), objectName);
                }
            } catch (InstanceAlreadyExistsException e) {
        ...
        private ObjectName createName(String type, MetricId id) {
            String domain = id.getGroup();
    ...
    private static class MetricTimeUnits {
        ...
        private final Map<MetricId, TimeUnit> rateOverrides;
        private final Map<MetricId, TimeUnit> durationOverrides;

        MetricTimeUnits(TimeUnit defaultRate,
                        TimeUnit defaultDuration,
                        Map<MetricId, TimeUnit> rateOverrides,
                        Map<MetricId, TimeUnit> durationOverrides) {
        ...
        public TimeUnit durationFor(MetricId id) {
            return durationOverrides.getOrDefault(id, defaultDuration);
        }

        public TimeUnit rateFor(MetricId id) {
            return rateOverrides.getOrDefault(id, defaultRate);
        }
    }
