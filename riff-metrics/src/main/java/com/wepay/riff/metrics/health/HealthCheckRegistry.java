...
import com.wepay.riff.metrics.health.annotation.Async;
...
import static com.wepay.riff.metrics.health.HealthCheck.Result;
...
public class HealthCheckRegistry {
     ...
     * @return a map of the health check results
     */
    public SortedMap<String, Result> runHealthChecks(HealthCheckFilter filter) {
     ...
     * @return a map of the health check results
     */
    public SortedMap<String, Result> runHealthChecks(ExecutorService executor) {
     ...
     * @return a map of the health check results
     */
    public SortedMap<String, Result> runHealthChecks(ExecutorService executor, HealthCheckFilter filter) {
        ...
        for (Map.Entry<String, Future<Result>> entry : futures.entrySet()) {
            try {
                ...
                results.put(entry.getKey(), Result.unhealthy(e));
            }
        }
